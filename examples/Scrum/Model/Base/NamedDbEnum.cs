...
namespace Scrum.Model.Base
{
	...
	{
	    private static T[] _allDbEnums;

	    public static IReadOnlyList<T> AllDbEnums
	    {
	        get
	        {
	            if (_allDbEnums == null)
	            {
	                var members = typeof(T).GetProperties(BindingFlags.Public | BindingFlags.Static)
                        .Select(mi => mi.GetGetMethod().Invoke(null, new object[0]))
                        .OfType<T>()
                        .ToArray();
                    _allDbEnums = members;
	            }
